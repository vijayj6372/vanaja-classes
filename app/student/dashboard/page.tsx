...
function ActivityGrid({ logs }: { logs: ActivityLog[] }) {
  ...
  return (
    <div className="grid grid-cols-7 gap-1.5">
      {cells.map((cell, i) => (
        <div key={i} title={`${cell.date}: ${cell.count} activities`}
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!meRes.ok) { router.push('/student/login'); return; }
      const { student: me } = await meRes.json();
      setStudent(me);

      const [logsRes, rankRes] = await Promise.all([
        fetch(`/api/student/activity?id=${me.id}`),
        fetch('/api/student/rank?id=' + me.id),
      ]);
