    ...
    <AdminLayout>
      <div className="p-4 sm:p-6 lg:p-8">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl sm:text-2xl font-black">Question Manager</h2>
          <button onClick={() => { setForm(emptyForm); setEditId(null); setShowModal(true); }}
            className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white font-bold px-4 py-2.5 sm:px-5 sm:py-3 rounded-xl transition-all text-sm sm:text-base">
            <Plus size={16} /> <span className="hidden sm:inline">Add Question</span><span className="sm:hidden">Add</span>
          </button>
        </div>

        <div className="flex gap-3 mb-5 flex-wrap">
          <select value={filterStd} onChange={e => setFilterStd(e.target.value)}
            className="bg-slate-800 border border-slate-700 rounded-xl px-3 py-2.5 text-sm font-medium focus:outline-none focus:border-purple-500 flex-1 min-w-[120px]">
        ...
        <div className="space-y-3">
          {questions.map((q, i) => (
            <motion.div key={q.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
              className="bg-slate-800 border border-slate-700 rounded-2xl p-4 sm:p-5 flex items-start gap-3 sm:gap-4">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-white font-semibold text-sm leading-snug mb-2">{q.question}</p>
        ...
        {showModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ y: '100%', opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: '100%', opacity: 0 }}
              ...
              className="sm:rounded-3xl bg-slate-800 p-5 sm:p-8 w-full sm:max-w-xl max-h-[92vh] overflow-y-auto border border-slate-700">
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-lg font-black">{editId ? 'Edit Question' : 'Add Question'}</h3>
              ...
              <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    {form.options.map((opt, i) => (
                      <div key={i} className="flex items-center gap-2">
                        <span className="text-slate-500 text-sm font-bold w-5">{String.fromCharCode(65 + i)}.</span>
                ...
                <button onClick={handleSave} disabled={loading}
                  className="w-full bg-purple-600 hover:bg-purple-700 text-white font-black py-3.5 rounded-2xl transition-all disabled:opacity-50">
